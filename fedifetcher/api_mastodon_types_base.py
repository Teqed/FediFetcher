...
class MaybeSnowflakeIdType(str):
    ...
    def __init__(self, val: PrimitiveIdType | datetime, assume_pleroma: bool = False) -> None:
        ...
        if isinstance(val, int | str):
            ...
            msg = f"Expected int or str, got {type(val).__name__}"
            raise TypeError(msg)
    ...
    try:
        if real_issubclass(t, AttribAccessDict):
            ...
            if isinstance(value, int):
                ...
                try:
                    value_int = int(value)
                    value = datetime.fromtimestamp(value_int, UTC)
                except:
                    try:
        ...
        if "__annotations__" in self.__class__.__dict__:
            for attr, _ in self.__class__.__annotations__.items():
                ...
                if hasattr(self.__class__, "_rename_map"):
                    attr_name = self.__class__._rename_map.get(attr, attr)
                    if attr_name in kwargs:
                        self[attr] = kwargs[attr_name]
                        assert attr not in kwargs, f"Duplicate attribute {attr}"
                elif attr in kwargs:
    ...
    def __getattr__(self, attr):
        ...
        if not hasattr(self.__class__, "_access_map"):
            ...
            if attr in self and self[attr] is not None:
                return self[attr]
            elif attr in self.__class__._access_map:
                try:
                    attr_path = self.__class__._access_map[attr].split(".")
                    cur_attr = self
