...
def increment_count(
    ...
    if (
        ...
        trending_posts_dict[post_url]["reblogs_count"] = incrementing_post[
            "reblogs_count"
        ]
    trending_posts_dict[post_url]["favourites_count"] += incrementing_post[
        "favourites_count"
    ]


def add_post_to_dict(
    trending_post: dict[str, str],
...
class VariableManipulators:
    ...
    def add_to_remembering(self, fetch_domain: str, status_id: str) -> None:
    ...
    def remove_from_remembering(self, fetch_domain: str, status_id: str) -> None:
...
async def fetch_and_return_missing(
    external_tokens: dict[str, str],
    trending_posts_dict: dict[str, dict[str, str]],
    ...
    async with semaphore:
        await fetch_trending_from_domain(
            external_tokens,
            add_post_to_dict,
...
async def aux_domain_fetch(
    external_tokens: dict[str, str],
    add_post_to_dict: Callable[[dict[str, str], str, dict[str, dict[str, str]]], bool],
    domains_fetched: list[str],
    post_urls: list[str],
    ...
    async with semaphore:
        ...
        for post_url in post_urls:
            ...
            if parsed and parsed[0] and parsed[0] == parsed_urls[0][0] and parsed[1]:
                remote = await api_mastodon.Mastodon(
                    parsed[0],
                    external_tokens.get(parsed[0]),
                ).get_status_by_id(parsed[1])
    ...
    if trending_posts:
        for post in trending_posts:
            ...
            if parsed_url[0] in domains_to_fetch:
                ...
                if (
                    parsed_url[0] not in remembering
                    or parsed_url[1] not in remembering[parsed_url[0]]
                ):
            ...
            if parsed_url[0] not in domains_fetched:
                ...
                remote = await api_mastodon.Mastodon(
                    parsed_url[0],
                    external_tokens.get(parsed_url[0]),
                ).get_status_by_id(parsed_url[1])
                if remote and remote["url"] == post_url:
...
async def update_local_status_ids(
    trending_posts_dict: dict[str, dict[str, str]],
    home_server: str,
    home_token: str,
    pgupdater: PostgreSQLUpdater,
